...
int main(){
	...
	//int arr[] = { 1, 2, 3, 4 };
	//printf("%d\n",sizeof(arr));  //这里输出的是字节数
	//printf("%d\n",sizeof(arr+0));  //指针用法
	//printf("%d\n",sizeof(*arr));  //这里的指针指向首元素的量级,相当于sizeof(int);
	//printf("%d\n",sizeof(&arr));
	
	//printf("%d\n",sizeof(*&arr));  //先是数组指针,然后进行解引用,成为了原本的指针
	//printf("%d\n",sizeof(&*arr));  //先算*arr会先转成int*,再进行解引用成为了单纯的int

	//printf("%d\n",sizeof(&arr+1));  //所求是数组指针    这里的+1-1并不影响指针类型
	...
	//printf("%d\n",sizeof(arr));
	//printf("%d\n", sizeof(arr+1));    //当发生+则会自动转成指针,32位则直接是4即可
	//printf("%d\n",sizeof(*arr));
	//printf("%d\n",sizeof(&*arr));    //转成指针
